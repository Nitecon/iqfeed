...
func (f *FundamentalMsg) UnMarshall(d []byte) {
	...
	f.PayDate = GetDateMMDDCCYY(items[11])             // 02/11/2016,
	f.ExDivDate = GetDateMMDDCCYY(items[12])           // 02/04/2016,
	...
	f.BalSheetDate = GetDateMMDDCCYY(items[30])        // 12/31/2015,
	...
	f.Fifty2WkHighDate = GetDateMMDDCCYY(items[44])    // 04/28/2015,
	f.Fifty2WkLowDate = GetDateMMDDCCYY(items[45])     // 08/24/2015,
	f.CalYearHighDate = GetDateMMDDCCYY(items[46])     // 01/05/2016,
	f.CalYearLowDate = GetDateMMDDCCYY(items[47])      // 01/28/2016,
