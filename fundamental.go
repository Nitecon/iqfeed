...
func (f *FundamentalMsg) UnMarshall(d []byte, tz string) {
	...
	f.PayDate = GetDateMMDDCCYY(items[11], tz)          // 02/11/2016,
	f.ExDivDate = GetDateMMDDCCYY(items[12], tz)        // 02/04/2016,
	...
	f.BalSheetDate = GetDateMMDDCCYY(items[30], tz)     // 12/31/2015,
	...
	f.Fifty2WkHighDate = GetDateMMDDCCYY(items[44], tz) // 04/28/2015,
	f.Fifty2WkLowDate = GetDateMMDDCCYY(items[45], tz)  // 08/24/2015,
	f.CalYearHighDate = GetDateMMDDCCYY(items[46], tz)  // 01/05/2016,
	f.CalYearLowDate = GetDateMMDDCCYY(items[47], tz)   // 01/28/2016,
	f.YrEndClose = GetFloatFromStr(items[48])           // 105.26,
	f.MaturityDate = GetDateMMDDCCYY(items[49], tz)     // ,
	f.CouponRate = GetFloatFromStr(items[50])           // ,
	f.ExpirationDate = GetDateMMDDCCYY(items[51], tz)   // ,
